-- Comprehensive schema cleanup and standardization
-- This migration ensures all tables have the columns expected by the application

-- First, ensure all missing columns exist in landing_pages
...
ALTER TABLE landing_pages 
ADD COLUMN IF NOT EXISTS theme_side TEXT;
...
-- Ensure all missing columns exist in services
...
-- Ensure image_urls column exists and is proper array type
ALTER TABLE services 
ADD COLUMN IF NOT EXISTS image_urls TEXT[] DEFAULT ARRAY[]::TEXT[];

-- Ensure all missing columns exist in highlights  
...
-- Ensure all missing columns exist in testimonials
...
-- Ensure user_pro_status has all required columns
ALTER TABLE user_pro_status 
ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT NOW();

ALTER TABLE user_pro_status 
ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW();

-- Now create a robust RPC function based on confirmed schema
