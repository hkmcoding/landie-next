-- Fix missing bio, cta, and social fields in get_dashboard_data_optimized RPC function
-- Only modify the landing page query to add the missing fields

-- First, let's get the current working function and modify just the landing page part
...
BEGIN
  ...
  -- Get aggregated services (keep original working version)
  ...
  -- Get aggregated highlights (keep original working version)
  SELECT COALESCE(json_agg(json_build_object(
    'id', id, 
    'header', header, 
    'content', content,
  ...
  -- Get aggregated testimonials (keep original working version)  
  SELECT COALESCE(json_agg(json_build_object(
    'id', id, 
    'quote', quote, 
